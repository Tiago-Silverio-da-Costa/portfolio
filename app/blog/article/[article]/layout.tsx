...
export const generateMetadata = async ({ params }: { params: { article: string } }): Promise<Metadata> => {
	...
	return {
		...
		metadataBase: new URL(`http://localhost:3000/article/${id}`),
		description:
		...
		openGraph: {
			...
			images: [
				{
					url: data?.image || "http://localhost:3000/tiagosc-port.png",
					width: 1280,
					height: 720,
				},
			],
		...
		twitter: {
			...
			description:
				data?.subtitle || "Blog pessoal com artigos sobre tecnologia, programação e desenvolvimento web.",
			images: [data?.image || "http://localhost:3000/tiagosc-port.png"],
		},
	};
};
