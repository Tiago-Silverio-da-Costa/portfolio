...
export default function Experience() {
    ...
    const deleteExperience = async (id: number) => {
        try {
            await fetch(`https://us-central1-portfolio-backend-34b37.cloudfunctions.net/api/deletexp/${id}`, {
                credentials: "include",
                cache: "no-cache",
                method: "DELETE",
                headers: {
    ...
    const onSubmit = async (data: TCreateExperience) => {
        ...
        const responseData = await fetch("https://us-central1-portfolio-backend-34b37.cloudfunctions.net/api/createxp", {
            credentials: "include",
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                ...
                <div className={`${selectedJob === null ? "w-full" : "w-fit"} flex gap-4 flex-col`}>
                    {experience?.map((job) => (
                        <div
                            ...
                            className="bg-textTitle text-bgFooter rounded-md p-4 flex flex-col hover:bg-textTitle/80 cursor-pointer transition-all duration-200">
